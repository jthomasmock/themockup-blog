---
title: "reactable - interactive tables guide"
description: |
...
```{r show_col viridis}
scales::show_col(viridis::viridis(n = 16))
```

`scales::show_col()` also works with `paletteer` to display palettes/vectors.

```{r show_col viridis}
scales::show_col(paletteer::paletteer_d("ggsci::red_material", n = 9))
```
...
```{r viridis show_col}
scales::show_col(viridis::viridis(12, begin = 0.5, direction = -1))
```
...
Note that the above table is essentially `done`. It has all the core changes we wanted (conditional color and interactivity). Everything below is `extra` stuff that I am also adapting from Greg's example with the Women's World Cup table.
...
I want to use a Google font (Karla + Fira Mono) - you can see all the [Google Fonts here](https://fonts.google.com/), search for specific fonts, Select the Style you want (+ button), and then [open the sidebar](https://fonts.google.com/specimen/Karla?selection.family=Fira+Mono:wght@500|Karla:ital@1&query=karla&sidebar.open) to extract the API call for embedding.
