...
I also have owed this post to many people, namely Maelle Salmon and JD Long as seen [here](https://twitter.com/thomas_mock/status/1287936723843002368?s=20).

## Why consider `distill`? 

No Hugo dependency, no theme dependency, it’s a very stable format with the ability to customize just enough with a bit of effort. Think a self-hosted `Medium.com` in appearance. Very minimal, lightly themed, focused on writing and code! 

I am always amazed at the power of both [`blogdown`](https://github.com/rstudio/blogdown) and the more recent [`hugodown`](https://hugodown.r-lib.org/), but you are still relying on a changing version of Hugo and your theme over time. For a personal blog I personally prefer `distill` because I can get busy, neglect my blog for a year, come back to write a new post, and it still *`just works`*.

However, `blogdown` and `hugodown` allow dramatically more customization. `distill` is truly an **opinionated** framework, and most of the *easy* customization you can do is fonts, colors, and the navbar. If you expect to create gorgeous sites like [education.rstudio.com](education.rstudio.com) or something like Desirée de Leon's [blog](https://desiree.rbind.io/) you’re likely going to be disappointed!

Again, for my blog I really just wanted a simple but IMO elegant minimalist theme, so I was fine with the tradeoff.
