...
To create verbatim code chunks you'll need to wrap the three backticks of the code chunk with _four_ backticks on either side. You _also_ need to invalidate the code chunk by adding a blank `r knitr::inline_expr(" ''")` to the _end_ of the existing code chunk (ie immediately after the `{r}` like `{r}1`r''`).
...
As a code example I can wrap my code in 5x backticks to create the below which displays 4x backticks around the 3x backticks:
