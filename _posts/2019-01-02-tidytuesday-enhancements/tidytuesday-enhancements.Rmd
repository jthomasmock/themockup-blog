...
We can refactor to a function AND since I only post these tweets on the Monday before `#TidyTuesday`, I can build variable dates into the function. Really the only thing I need to manually change is **`what data we are exploring`** and **`the long article link`** to be shortened.
