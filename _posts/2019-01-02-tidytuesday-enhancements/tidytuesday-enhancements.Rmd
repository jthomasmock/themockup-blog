...
One of my `#rstats` goals for 2019 was to make #TidyTuesday better, both from the experience on GitHub and the weekly posting for myself and for others. The first thing I did for the project was to re-organize the GitHub repo, moving all the 2018 data into an archived folder and making a new 2019 folder to begin populating with new data! Additionally, with some help from Phillip Khor via a pull-request - the GitHub repo is now full of relative (rather than absolute) paths to make life easier for anyone that forks the repo. 
...
I generally switched between a lot of tabs, between GitHub, links, pics, etc while getting the weekly post setup. I knew there was a way to at least partially automate this, so I looked into my options.

The `rtweet` package has a post to Twitter function via `rtweet::post_tweet()` which allows you to programmatically post tweets! However - to get where I wanted to be I had to add a few more things and do some interesting troubleshooting!
...
This is not exactly what I want... the text isn't aligned properly, I want at least more than 1 pic, and I really would prefer to use emoji to save characters over `Data:` and `Article:`.

So on to the next step!

Hadley Wickham has the [`emo`](https://github.com/hadley/emo) package which allows you to input various emoji into R/R Markdown. This package perfectly solves our emoji needs! I can use code like `emo::ji("folder")` to get `r emo::ji("folder")` or `emo::ji("news")` to get `r emo::ji("news")`! But I still need to get my text aligned properly. I broke some formatting rules for normal code, but it gets the job done for this `rtweet` use! Basically, I forced the text to be fully left-aligned, added in some spacing, and used `paste0()` to get everything squished together in a coherent tweet (with multiple pics!).
...
- `We're exploring {topic of interest}!`  
...
Since we are going to be linking local pics, I'm using the [`here`](https://github.com/r-lib/here) package to make moving between folders in my project easier. My folder is organized like so:
...
short_link <- "http://tinycc/WHATEVER"
...
However, we can refactor things further! We can refactor to a function AND I only post these tweets on the Monday before `#TidyTuesday` so I can build variable dates into the function. Really the only thing I need to manually change is **`what we are exploring`** and **`long article link`**.
...
post_tidytuesday <- function(exploring, long_link){
        ...
        short_link <- tinycc::shorten(longURL = long_link,
                                      # Use glue to paste the week num  
                                      # to get useful defined shortlinks
                                      shortURL = glue::glue("tt_2019_{week_num}")) %>%
                # get the short URL (nested list is original storage)
                purrr::pluck("results", "short_url") 
        
        # post the tweet with filled input
        # Notice that {week_num} is automatically generated, while {exploring} is manually defined
        rtweet::post_tweet(status = glue::glue(
                "The @R4DScommunity welcomes you to week {week_num} of #TidyTuesday!  We're exploring {exploring}!
...
#r4ds #tidyverse #rstats #dataviz"),
                ...
                media = c(here::here("static_img", "tt_logo.png"),
                          ...
                          # Week_date is automatically defined 
                          # by the week_date input above
...
post_tidytuesday(
  exploring = "Esoteric data!",
  long_link = "http://cool.article.com/long-title-with-lots-of-words"
)
```
