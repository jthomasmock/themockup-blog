...
If you want to go deeper than this basic guide, check out the [`gt` site](https://glin.github.io/reactable/articles/examples.html), which has lots of examples!
...
We covered conditional colors in the previous [post](https:themockup.blog/) - check it out if you have further questions about building or using palettes.

The MAJOR value add that `gt` provides is rather than having to write our own palette parser it has a built in one via `gt::data_color()`! Just to refresh us on our palette, we're using an adaptation of the `viridis` palette.
