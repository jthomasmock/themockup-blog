...
dog %>% 
  ...
  image_composite(bird, offset = "+50+100")
```

Notice that offset reads as x-axis pixels, y-axis pixels, so from the top left corner place the image 50 pixels to the right and 100 pixels down. 
...
In the exampe I will show below you will have already saved the plot as an image, so we can skip with the normal `ggplot2` code. However, an interesting note is that if you do want to build in some white to your plot to save "space" for your logo you can do so by changing the margins of your overall plot. Margins can take multiple units arguments including: "cm", "in", "lines", "pt", for more info check out the [`ggplot2` docs](https://ggplot2.tidyverse.org/reference/element.html).
...
Notice the below plot has extra space on the bottom.
...
So let's read in our publication plot to work with! Notice that this can be a local file with traditional path structure or a hosted image as a url.

```{r, echo = TRUE}
pub_plot <- image_read("https://pbs.twimg.com/media/DwaRAv6U0AAaLIx.jpg")

logo <- image_read("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/static/plot_logo.png") %>% 
  image_resize(120)

print(pub_plot)
print(logo)
```
We can then overlay the logo over the plot, let's try bottom left, by using `offset = "+12+840"` indicating placing the logo 12 pixels to the right and 840 pixels down.

```{r, echo = TRUE}
pub_plot %>% 
  image_composite(logo, offset = "+12+840")
```

Boom! We have our logo overlay on the plot in the right location! But we had to manually set the logo position, which is less than ideal for programmatic use down the road.

## Put it in a function! (Bird not included)

We can build a function to take the various heights and widths of the logos and automatically resize the logo to match.

This function takes in two images, gets the dimension information, then pastes the logo at 1/10th scale in the specified corner with 1% padding.
...
The function with a warning if you input a logo_position that is not included.
