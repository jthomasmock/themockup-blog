---
title: "RMarkdown - how I share knowledge around R Markdown"
author: "Thomas Mock"
date: "7/25/2020"
...
She saves the following topics for later or longer sessions:  

- **`render()`** - programmatic building of R Markdown.  
- **Parameters** - she uses these but in focused activity.  
- **Multiple Rmd formats** - Covers these in longer formats
- **`Markdown`** - Skims this, "it's dry", and should probably be self-directed learning as needed.
...
- Exploratory Data Analysis w/ dialogue - Dave Robinson's `#TidyTuesday` [screencasts](https://www.youtube.com/user/safe4democracy/playlists) + [code](https://github.com/dgrtwo/data-screencasts)
...
- MVP of [Reproducibility](https://datacarpentry.org/rr-intro/02-toolkit/): Whether you talk about **M**inimum **V**iable **P**roduct or **M**ost **V**aluable **P**layer, it works!  
  - R Markdown **HAS** to run to save/knit the output  
...
### [Entire Websites](https://bookdown.org/yihui/rmarkdown/websites.html)  
  ...
  - [`distill`](https://rstudio.github.io/distill/) for scientific writing, native to the web  
...
Less common, but just as important is the idea of R Markdown as a meta-document that lets you bring in other code or automate processes.
...
- Parameters are data passes to the R Markdown document to generate new outputs from the same code, an example here is generating a custom report for all 50 US states.  
...
- [Emailing w/ `blastula`](https://solutions.rstudio.com/examples/blastula/conditional-example/) - maybe your boss is too busy to consume a full report every day - send a conditional email directly to them if a specific number is hit or missed all with code in R! This email could is built with R Markdown, and could contain plots, tables, raw data, or ANY R Markdown-based document (so... basically anything).  
...
Minimal example below with the [`palmerpenguins` dataset](https://github.com/allisonhorst/palmerpenguins). Placed below as an image due to nesting R Markdown chunks inside R Markdown chunks in a R Markdown-based website. Full code at: [https://git.io/JJBcC](https://git.io/JJBcC).
...
### [whisker](https://github.com/edwindj/whisker)  
  ...
  - It uses `glue` style syntax to add data to templates either in memory or to an output file.  
...
Sharla Gelfand, the "resident reproducible reporter" put together lots of material using the `usethis::use_template()` workflow, which I'm now dubbing the `Sharla Strategy`.

- [Sharla Gelfand's `use_template()` blogpost](https://sharla.party/post/usethis-for-reporting/)  
  - In short, she turned an annual report from a mess of copy-pasting and manual work into a fast, streamlined, reproducible, and easily repeatable workflow using R, R Markdown, and `usethis::use_template()`.  
  - [Recording](https://rstudio.com/resources/rstudioconf-2020/don-t-repeat-yourself-talk-to-yourself-repeated-reporting-in-the-r-universe/) of her presentation on the same topic  
