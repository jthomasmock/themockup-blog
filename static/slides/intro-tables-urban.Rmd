...
```{r, echo = FALSE, fig.retina=2, out.width="75%"}
...
```{r, echo = FALSE,eval = FALSE, fig.retina=2, out.width="75%"}
tab_538 <- espn_qbr %>% 
  mutate(name = paste0(
    "<span style='font-size:16px; color:royalblue;'>",
    name,
...
```{r, echo = FALSE, fig.retina=2, out.width="75%"}
knitr::include_graphics("images/urban/538-screen-tab.png")
```
...
```{r, eval = FALSE, echo = FALSE, fig.retina=2, out.width="75%"}
library(tidyverse)
library(rvest)
library(gt)

raw_url <- "https://fivethirtyeight.com/features/the-ideas-that-are-reshaping-the-democratic-party-and-america/amp/"
...
raw_html %>% 
  html_node("amp-iframe") %>% 
  html_attr("srcdoc") %>% 
  read_html() %>% 
  html_table() %>% 
  .[[1]] %>% 
  set_names(nm = c("Position", "dem_16", "all_16", "dem_20", "all_20")) %>% 
  slice(-1) %>% 
  gt() %>% 
  tab_header(
    title = md("**Democrats have embraced equality-focused positions**"),
    subtitle = "Share of Democratic voters that support each position according to polling"
  ) %>% 
  cols_label(
    dem_16 = "DEMS",
    all_16 = "ALL",
  ...
  opt_all_caps() %>% 
  cols_width(
    vars(Position) ~ px(600),
    everything() ~ px(50)
  ) %>% 
  opt_table_font(google_font("Lato")) %>% 
  tab_spanner(
    label = md("PRE-2016\nELECTION"),
    columns = vars(dem_16, all_16),
    id = "election"
  ) %>% 
  tab_spanner(
    label = "2019-2020",
    columns = vars(dem_20, all_20),
    id = "year"
  ) %>% 
  tab_style(
  ...
  tab_style(
    ...
    locations = cells_column_spanners(c("election", "year"))
  ) %>% 
  tab_style(
    style = cell_text(font = google_font("Fira Mono"), size = px(14)),
    locations = cells_body(columns = vars(dem_16, all_16, dem_20, all_20))
  ) %>% 
  tab_style(
    style = list(
      cell_borders(sides = "left", weight = px(2)),
      cell_fill(color = "#EEEEEE")
    ),
    locations = cells_body(columns = vars(dem_16, dem_20))
  ) %>% 
  tab_style(
    style = cell_borders(sides = "left", weight = px(5), color = "white"),
    locations = cells_column_spanners("year")
  ) %>%
  tab_style(
    style = cell_borders(sides = "bottom", color = "lightgrey"),
    locations = cells_body()
  ) %>% 
