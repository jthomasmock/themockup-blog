...
You may typically use something like `Jan:Dec`, but this won't work in `vars()`. This throws an error `Error: Can't convert a call to a string`. However we can accomplish similar things through other techniques.
...
We can compare the code for the below, note the use of `month.abb` on the left or ALL the column names on the right.
...
Now you can skip using `vars()` and the named column and just supply the column number.
...
You can subset the names of the column but must assign it to an object to use it inside `vars()`.
...
You can pass raw strings `vars()` and separate them by commas.
