...
There are a lot of available functions in `gtExtras`:
...
Importantly, `gtExtras` is not at all a _replacement_ for `gt`, but rather is almost a "cookbook" where common or repeated function calls are grouped into their own respective functions. At a technical level, `gtExtras` is literally just `gt` functions under the hood and I'll highlight a few examples of how to do the same thing in each package. 

I ultimately hope that seeing some of the easy things you can create with `gt` and `gtExtras` inspires you to create your _own_ R package or opinionated functions to use in your work.
...
`gt_theme_guardian()` styled after ESPN style tables, and the
...
The goal here is creating a theme that works for most of your needs, so you don't have to recreate the wheel each time.
...
Some complexity arises when you decide to use a color palette with `scales` for tighter control. The code below is easy enough but requires you to write out a lot of essentially boilerplate code that doesn't vary much for different use cases.
...
`gtExtras::gt_color_rows()` is also simple to use but out of the box provides rich color
...
Note that it is very close to the `color_fn` from above but throws a useful warning if domain is `NULL` since that will use range within EACH column rather than a shared range ACROSS columns. This is more important when your columns are related like a heatmap than this simple demo.
...
There's a LOT of palettes!

```{r}
paletteer::palettes_d_names |> 
  dplyr::count(package, type, sort = TRUE)
```
...
```{r}
gt(head(mtcars)) |> 
  gt_color_rows(cyl, palette.colors(n = 3), domain = c(4,6,8))

```

Again, this function is an example of something that is easily possible with `gt` but would require a good chunk of repeated boilerplate code. The biggest value is thinking only about palettes/colors instead of `scales::col_factor()/col_numeric()`

If you wanted to use categorical palettes with `gt`, you could so like below:

```{r}
cat_color <- scales::col_factor(
  palette = palette.colors(n = 3), 
  domain = c(4,6,8))

gt(head(mtcars)) |> 
  data_color(cyl, cat_color)
```
