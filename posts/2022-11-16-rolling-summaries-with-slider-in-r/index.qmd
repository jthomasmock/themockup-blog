...
We can return a list of equal length to the input, that accumulates the current row plus 2 values before it. Note that it _slides_ across the values and won't return more than the 3 requested (current row + 2 rows before).
